...
export const NotificationSchemeModal: React.FC<Props> = ({ isOpen, currentScheme, onClose, onSave }) => {
      ...
      <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-slideUp flex flex-col max-h-[85vh]">
        ...
        <div className="p-6 space-y-4 overflow-y-auto">
           <div className="space-y-4">
              {EVENTS.map((event) => (
                <div key={event.id} className="py-3 border-b border-gray-50 last:border-0">
                   <div className="font-bold text-gray-800 text-sm mb-3">{event.label}</div>
                   <div className="flex gap-3">
                      {RECIPIENTS.map(r => {
                        ...
                        return (
                          ...
                          >
                            {r === 'Reporter' ? '報告者' : r === 'Assignee' ? '担当者' : 'ウォッチャー'}
                          </button>
                        );
                      })}
                   </div>
        ...
        <div className="px-6 py-4 bg-gray-50 flex gap-3 justify-end shrink-0 border-t border-gray-100">
           ...
           <button onClick={() => onSave(scheme)} className="px-4 py-2 bg-primary text-white rounded-lg text-sm font-bold shadow-lg hover:bg-primaryHover">保存</button>
        </div>
      </div>
    </div>
  );
