...
export const WorkflowEditorModal: React.FC<Props> = ({ isOpen, currentWorkflow, onClose, onSave }) => {
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-slideUp flex flex-col max-h-[85vh]">
             ...
             {Object.keys(STATUS_LABELS).map((from) => (
                <div key={from} className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                   <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="font-bold text-gray-800">{STATUS_LABELS[from as keyof typeof STATUS_LABELS] || from}</span>
                   </div>
                   <div className="space-y-2 pl-4 border-l-2 border-gray-200 ml-1.5">
                      {(workflow[from] || []).map(to => (
                        <div key={to} className="flex items-center justify-between text-sm text-gray-600 group bg-white p-2 rounded border border-gray-100">
                           <div className="flex items-center gap-2">
                             <ArrowRight size={14} className="text-gray-400"/>
                             <span>{STATUS_LABELS[to as keyof typeof STATUS_LABELS] || to}</span>
                           </div>
                           <button onClick={() => handleRemoveTransition(from, to)} className="text-red-400 opacity-0 group-hover:opacity-100 hover:text-red-600 transition-all p-1">
                             <Trash2 size={12} />
                           </button>
                        </div>
                      ))}
                      ...
                      {editingFrom === from ? (
                        <div className="flex gap-2 items-center bg-white p-2 rounded border border-blue-200 animate-fadeIn">
                           <select 
                             ...
                             onChange={(e) => { if(e.target.value) handleAddTransition(from, e.target.value); }}
                             defaultValue=""
                           >
                              <option value="" disabled>遷移先を選択...</option>
                              {Object.keys(STATUS_LABELS).filter(s => s !== from && !(workflow[from] || []).includes(s)).map(s => (
                                <option key={s} value={s}>{STATUS_LABELS[s as keyof typeof STATUS_LABELS] || s}</option>
                              ))}
                           </select>
                           <button onClick={() => setEditingFrom(null)} className="text-gray-400"><X size={14}/></button>
                        </div>
                      ) : (
                        <button 
        ...
        <div className="px-6 py-4 bg-gray-50 flex gap-3 justify-end shrink-0 border-t border-gray-100">
           <button onClick={onClose} className="px-4 py-2 border border-gray-200 rounded-lg text-sm font-bold text-gray-500 hover:bg-white">キャンセル</button>
           <button onClick={() => onSave(workflow)} className="px-4 py-2 bg-primary text-white rounded-lg text-sm font-bold shadow-lg hover:bg-primaryHover">保存</button>
        </div>
      </div>
    </div>
  );
