...
import { Calendar, ChevronRight, Maximize2, Minimize2, ZoomIn, ZoomOut } from 'lucide-react';
...
export const ProjectTimeline: React.FC<Props> = ({ issues }) => {
  ...
  const getX = (dateStr?: string) => {
    ...
    const diffDays = (date.getTime() - timelineConfig.start.getTime()) / (1000 * 60 * 60 * 24);
  ...
  const labels = useMemo(() => {
    ...
    while (curr <= timelineConfig.end) {
      if (zoom === 'week') {
        res.push({ label: `${curr.getMonth()+1}/${curr.getDate()}`, left: getX(curr.toISOString()) });
        curr.setDate(curr.getDate() + 1);
      } else if (zoom === 'quarter') {
        if (curr.getDate() === 1) res.push({ label: `${curr.getMonth()+1}月`, left: getX(curr.toISOString()) });
  ...
  return (
    <div className="h-full flex flex-col bg-white overflow-hidden">
      ...
      <div className="flex-1 overflow-auto no-scrollbar" ref={containerRef}>
        <div style={{ width: timelineConfig.width + 300 }} className="relative p-6">
          <div className="flex sticky top-0 bg-white z-20 border-b border-gray-100 pb-4">
             <div className="w-[300px] shrink-0 font-black text-[9px] text-gray-400 uppercase tracking-widest">作業項目</div>
             <div className="flex-1 relative h-4">
                {labels.map((l, i) => (
                  <div key={i} className="absolute text-[8px] font-black text-gray-300 border-l border-gray-100 h-2 pl-1" style={{ left: l.left }}>{l.label}</div>
                ))}
             </div>
          </div>
          ...
          <div className="space-y-2 mt-4 relative z-0">
            {sortedIssues.map((issue) => {
              ...
              const endX = getX(issue.dueDate || new Date(new Date(issue.createdAt).getTime() + 7 * 86400000).toISOString());
              ...
              return (
                <div key={issue.id} className="flex items-center group h-9">
                  <div className="w-[300px] shrink-0 pr-6 flex items-center gap-3 overflow-hidden">
                     <span className="text-[9px] font-mono text-gray-400 shrink-0 tracking-tighter">{issue.key}</span>
                     <span className="text-xs font-bold text-gray-700 truncate group-hover:text-primary transition-colors">{issue.title}</span>
                  </div>
                  <div className="flex-1 relative h-6 bg-gray-50/30 rounded-lg">
                     ...
                     >
                       {width > 40 && <span className="text-[8px] text-white font-black truncate uppercase">{issue.key}</span>}
                     </div>
                     <div className="absolute top-0 pointer-events-none" style={{ left: endX + 8 }}>
