...
import { Plus, Users, LayoutList, ChevronDown, ChevronRight, Filter, UserCheck, Clock, AlertTriangle } from 'lucide-react';
import { STATUS_LABELS, updateIssueStatus, USERS, getCurrentUserId, db } from '../../services/mockData';
...
export const ProjectBoard: React.FC<Props> = ({ project, issues, onIssueClick, onCreateIssue }) => {
  const [swimlaneMode, setSwimlaneMode] = useState<'none' | 'assignee'>('none');
  const [collapsedSwimlanes, setCollapsedSwimlanes] = useState<Set<string>>(new Set());
  const [activeQuickFilters, setActiveQuickFilters] = useState<string[]>([]);
  const [isOverColumn, setIsOverColumn] = useState<{status: IssueStatus, swimlaneId: string} | null>(null);
  ...
  const getColumnLimit = (status: IssueStatus) => project.columnSettings?.[status]?.limit;
    ...
    <div className="h-full flex flex-col bg-bgLight md:bg-transparent overflow-hidden">
      <div className="px-4 py-3 border-b border-gray-200 flex flex-wrap items-center justify-between bg-white shrink-0 gap-3">
        ...
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
           <button 
             onClick={() => setActiveQuickFilters(prev => prev.includes('mine') ? prev.filter(x => x !== 'mine') : [...prev, 'mine'])}
           ...
           <button 
             onClick={() => setActiveQuickFilters(prev => prev.includes('recent') ? prev.filter(x => x !== 'recent') : [...prev, 'recent'])}
        ...
        <div className="flex gap-4 min-h-full">
          {statuses.map(status => {
            ...
            return (
              ...
              >
                {/* 列ヘッダー */}
                <div className={`p-3 font-bold text-[10px] uppercase tracking-widest flex justify-between items-center mb-3 rounded-lg ${isOverLimit ? 'bg-red-100 text-red-600' : 'bg-white border border-gray-200 text-gray-400'}`}>
                  <div className="flex items-center gap-2">
                    {STATUS_LABELS[status]} 
                    {isOverLimit && <AlertTriangle size={12} />}
                  </div>
                  <span className={`px-1.5 py-0.5 rounded-full ${isOverLimit ? 'bg-red-500 text-white' : 'bg-gray-100 text-gray-600'}`}>
                ...
                <div className="space-y-4">
                  {swimlanes.map(lane => {
                    const laneIssues = swimlaneMode === 'none' 
                      ? columnIssues 
                      : columnIssues.filter(i => (lane.id === 'unassigned' ? !i.assigneeId : i.assigneeId === lane.id));
                      ...
                      <div key={lane.id} className="space-y-2">
                        {swimlaneMode !== 'none' && status === statuses[0] && (
                          ...
                          >
                            ...
                            <span className="text-[10px] font-black uppercase text-gray-500 tracking-wider">{lane.name}</span>
                            <span className="text-[9px] bg-gray-200 px-1 rounded text-gray-500">{filteredIssues.filter(i => (lane.id === 'unassigned' ? !i.assigneeId : i.assigneeId === lane.id)).length}</span>
                          </div>
                        )}
                        
                        {!isCollapsed && (
                          <div className={`space-y-2 min-h-[40px] rounded-lg p-1 transition-colors ${isOverColumn?.status === status && isOverColumn?.swimlaneId === lane.id ? 'bg-blue-100/50' : ''}`}
                            onDragOver={(e) => { e.preventDefault(); setIsOverColumn({status, swimlaneId: lane.id}); }}
                            onDragLeave={() => setIsOverColumn(null)}
                          >
                            {laneIssues.map(issue => (
                              <div key={issue.id} draggable onDragStart={(e) => e.dataTransfer.setData('issueId', issue.id)} className="cursor-grab active:cursor-grabbing">
                  ...
                  {status === 'To Do' && (
                    <button onClick={() => onCreateIssue?.(status)} className="w-full py-2 flex items-center justify-center text-gray-400 hover:text-primary hover:bg-blue-50 rounded text-xs transition-colors border-2 border-dashed border-gray-200 hover:border-blue-200">
