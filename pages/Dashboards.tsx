...
export const Dashboards = () => {
  ...
  const projectData = projects.map(p => ({
    ...
    done: issues.filter(i => i.projectId === p.id && i.status === 'Done').length
  }));
  ...
  const GadgetWrapper: React.FC<{ id: string; title: string; icon: any; children?: React.ReactNode }> = ({ id, title, icon: Icon, children }) => (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden group">
      <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
        ...
        <button onClick={() => removeGadget(id)} className="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all p-1">
          <X size={16} />
        </button>
      </div>
      <div className="p-5">
    ...
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-6 pb-24">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">プロジェクトの概況をリアルタイムで監視します。</p>
        </div>
        <button 
            ...
            <GadgetWrapper key={id} id={id} title={g.title} icon={g.icon}>
                ...
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={projectData} layout="vertical" margin={{ left: -20 }}>
                      ...
                      <YAxis dataKey="name" type="category" width={40} fontSize={10} tickLine={false} axisLine={false} fontBold />
                      <Tooltip cursor={{fill: 'transparent'}} />
                      <Bar dataKey="done" stackId="a" fill="#36B37E" barSize={12} radius={[2, 0, 0, 2]} />
                      <Bar dataKey="count" stackId="a" fill="#DFE1E6" barSize={12} radius={[0, 2, 2, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              )}
              {id === 'bugs' && (
                <div className="space-y-2">
                  {issues.filter(i => i.type === 'Bug').slice(0, 4).map(bug => (
                    <div key={bug.id} className="flex items-center justify-between p-2.5 bg-gray-50 hover:bg-white border border-transparent hover:border-gray-200 rounded-xl transition-all">
                      <div className="flex items-center gap-3 overflow-hidden">
                        <div className="w-1.5 h-1.5 rounded-full bg-red-500 shrink-0" />
                        <span className="text-xs font-bold text-gray-700 truncate">{bug.title}</span>
                      </div>
                      <span className="text-[10px] font-mono text-gray-400 shrink-0">{bug.key}</span>
                    </div>
                  ))}
                </div>
              )}
              {id === 'due' && (
                <div className="space-y-2">
                  {issues.filter(i => i.dueDate && i.status !== 'Done').slice(0, 4).map(issue => (
                    <div key={issue.id} className="flex items-center justify-between p-2.5">
                      <span className="text-xs text-gray-700 truncate max-w-[180px] font-medium">{issue.title}</span>
                      <span className="text-[10px] font-bold text-orange-600 bg-orange-50 px-2 py-0.5 rounded-full">{new Date(issue.dueDate!).toLocaleDateString()}</span>
                    </div>
                  ))}
                </div>
              )}
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/50 backdrop-blur-sm animate-fadeIn">
           <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-slideUp">
              <div className="px-6 py-4 border-b flex justify-between items-center bg-gray-50">
                 <h3 className="font-bold text-gray-800 flex items-center gap-2"><List size={18} /> ガジェットを選択</h3>
                 <button onClick={() => setShowAddModal(false)} className="p-1 hover:bg-gray-200 rounded-full"><X size={20}/></button>
              </div>
              <div className="p-4 space-y-2">
                 {AVAILABLE_GADGETS.map(g => (
                   ...
                   >
                      <div className="w-10 h-10 bg-white border border-gray-100 rounded-lg flex items-center justify-center text-primary shadow-sm"><g.icon size={20}/></div>
                      <div className="flex-1">
                        <div className="text-sm font-bold text-gray-800">{g.title}</div>
                        <div className="text-xs text-gray-400">{activeGadgetIds.includes(g.id) ? '追加済み' : 'クリックして追加'}</div>
                      </div>
                   </button>
                 ))}
              </div>
