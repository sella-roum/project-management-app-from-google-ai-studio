...
export const Home = ({ onOpenIssue }: { onOpenIssue: (i: Issue) => void }) => {
  ...
  const myIssues =
    useLiveQuery(() => getIssuesForUser(getCurrentUserId())) || [];
    ...
    <div className="p-4 md:p-8 max-w-3xl mx-auto space-y-8 pb-32">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-secondary tracking-tight">
            {loading
              ? "読み込み中..."
              : `こんにちは、${user?.name.split(" ")[0]}さん`}
          </h1>
          <p className="text-gray-500 mt-1">今日のやるべきことはこちらです。</p>
        </div>
        <div className="flex items-center gap-2">
          <div
            className={`w-2.5 h-2.5 rounded-full ${isOnline ? "bg-green-500 animate-pulse" : "bg-gray-300"}`}
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          >
            {loading ? (
              <div className="h-6 w-10 bg-gray-100 rounded mx-auto animate-pulse" />
            ) : (
              <div className="text-xl font-black text-secondary">
                {stat.val}
              </div>
            )}
            <div className="text-[9px] text-gray-400 uppercase font-black mt-1 tracking-tighter">
              {stat.label}
            </div>
          </div>
        ))}
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between px-1">
          ...
          <button
            onClick={() =>
              navigate("/search", { state: { filter: "assigned" } })
            }
            className="text-[10px] text-primary font-black uppercase"
          >
            すべて
          </button>
        </div>
            ...
            myIssues.slice(0, 10).map((issue) => (
              ...
              >
                ...
                <div
                  ...
                  onTouchStart={(e) => {
                    (window as any)._startX = e.touches[0].clientX;
                  }}
                  onTouchEnd={(e) => {
                    const diff =
                      (window as any)._startX - e.changedTouches[0].clientX;
                ...
                >
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      setQuickActionIssue(
                        quickActionIssue === issue.id ? null : issue.id,
                      );
                    }}
                  ...
                  {quickActionIssue === issue.id && (
                    <div className="absolute top-12 right-2 bg-white border border-gray-100 shadow-2xl rounded-xl p-1 z-20 min-w-[120px] animate-slideInRight">
                      <button
                        ...
                        className="w-full text-left px-3 py-2 text-[10px] font-black text-gray-700 hover:bg-blue-50 rounded-lg flex items-center gap-2"
                      >
                        <Play size={12} className="text-blue-500" /> 進行中
                      </button>
                      <button
                        onClick={() => handleStatusChange(issue.id, "Done")}
                        className="w-full text-left px-3 py-2 text-[10px] font-black text-gray-700 hover:bg-green-50 rounded-lg flex items-center gap-2"
                      >
