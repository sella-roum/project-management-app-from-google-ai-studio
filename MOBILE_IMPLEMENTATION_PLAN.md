...
- [x] Summary/Board/Backlog のチャートや視覚表現をWebに合わせる（モバイルの簡易表示改善）。

## Phase 5.5: Dashboardsのチャート/ガジェット表現の同等化

- [x] Webのチャート/ガジェット表現（Pie/Bar等）をモバイルに反映。
  - 対象ファイル:
...
- [x] IssueCard 相当のコンポーネント化とメタ情報（アイコン/説明/タグ）を追加。
  - 対象ファイル:
...
- [x] Welcome/Login のレイアウトと文言をWebに合わせる。
  - 対象ファイル:
